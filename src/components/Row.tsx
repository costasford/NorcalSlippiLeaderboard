...
export function Row({ player }: Props) {
  ...
  const changeIndicator = (change: number, indicators: string[]) => (
    <span className={`px-1 md:text-sm text-xs ${change > 0 ? 'text-green-500' : 'text-red-500'}`}>
      {change > 0 ? indicators[0] : indicators[1]}
      {Math.abs(change)}
    </span>
  );

  const changeArrow = (change: number) => changeIndicator(change, ['▲ ', '▼ ']);
  ...
  const getRatingChange = (playerData: Player) => {
    if (!playerData.oldRankedNetplayProfile || !playerData.oldRankedNetplayProfile.ratingOrdinal) {
      return null;
    }
    return Math.floor(
      playerData.rankedNetplayProfile.ratingOrdinal
      - playerData.oldRankedNetplayProfile.ratingOrdinal,
    );
  };

  const playerRank = getRank(player);
  const isActive = playerRank.name !== 'None';
  const totalGames = (player.rankedNetplayProfile.characters || [])
    .reduce((acc, val) => acc + val.gameCount, 0);
    ...
    <tr className={`${playerRank.bgClass} border-separate border-spacing-2 border-b-2 border-gray-600`}>
      ...
      <td className="md:text-xl text-sm text-gray-900 md:px-6 md:py-4 p-1 whitespace-nowrap text-center">
        ...
        <div className="flex items-center justify-center">
          <img className="md:h-10 md:w-10 h-6 w-6 drop-shadow" src={playerRank.iconUrl} alt={`${playerRank.name} rank icon`} />
        </div>
        )}
      ...
      <td className="md:text-xl text-gray-300 text-sm md:px-6 md:py-4 md:p-1 whitespace-nowrap">
        {Boolean(totalGames) && (
        <>
          <span className="text-green-500">{player.rankedNetplayProfile.wins ?? 0}</span>
